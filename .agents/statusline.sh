...
# ─── Parse JSON from stdin (Single jq pass for performance) ──────────────────
{
  ...
  jq -r '
    ...
    (.context_window.tokens_used // .context_window.used_tokens // (.context_window.total_input_tokens + .context_window.total_output_tokens) // 0),
    (.context_window.tokens_total // .context_window.total_tokens // .context_window.context_window_size // 0),
    (.vcs.branch // ""),
    (.vcs.dirty // false),
...
# Helper function to format large numbers to 'k' or 'm' shorthand
fmt_tokens() {
  local num=$1
  if [ -z "$num" ] || [ "$num" -eq 0 ]; then
    echo "0"
  elif [ "$num" -ge 1000000 ]; then
    LC_NUMERIC=C printf "%.0fm" "$(echo "scale=1; $num / 1000000" | bc)"
  elif [ "$num" -ge 1000 ]; then
    LC_NUMERIC=C printf "%.0fk" "$(echo "scale=1; $num / 1000" | bc)"
  else
    echo "$num"
  fi
}

TOK_U_FMT=$(fmt_tokens "$TOK_USED")
TOK_T_FMT=$(fmt_tokens "$TOK_TOTAL")
...
# ─── Stats ───────────────────────────────────────────────────────────────────
CTX="${FG_GRAY}ctx ${BAR_COLOR}${BAR} ${NUM_COLOR}${PCT_FMT}% (${TOK_U_FMT}/${TOK_T_FMT})${R}"
